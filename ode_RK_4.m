...
dt_backwards = -dtime;    %cut dt_back in half to get midpts in back sweep

for i = n:-1:2

%     dv1 = rhs_forward(v(i), time(i)) - f(time(i));
%     dv2 = rhs_forward(v(i) + 0.5 * dtime * dv1, time(i) - 0.5 * dtime) - f(time(i) - 0.5 * dtime);
%     dv3 = rhs_forward(v(i) + 0.5 * dtime * dv2, time(i) - 0.5 * dtime) - f(time(i) - 0.5 * dtime);
%     dv4 = rhs_forward(v(i) + dtime * dv3,       time(i) - dtime)       - f(time(i) - dtime);
% 
%     v(i-1) = v(i) + dtime * (1/6) * (dv1 + 2*dv2 + 2*dv3 + dv4);
