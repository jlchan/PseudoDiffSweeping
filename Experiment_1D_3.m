...
%varying omega, testing 4 values of ppw (5, 10, 15)
...
for i = 25:25:200
    ...
    semilogy(omega,max(abs(err_RK4)),'ko','MarkerSize',10)
    hold on
...
for i = 25:25:200
    ...
    %hold on
    semilogy(omega,max(abs(err_RK4)),'bo','MarkerSize',10)
...
for i = 25:25:200
    ...
    semilogy(omega,max(abs(err_RK4)),'ro','MarkerSize',10)

end


%plot corresponding order of 1/omega curve

omega_vals = 25*pi:25*pi:200*pi;

%plot(omega_vals,1./omega_vals,'g-')     %sinusoidal wave speed

%plot(omega_vals,.4*(1./omega_vals),'g-')     %exponential wave speed

plot(omega_vals,-(1./omega_vals)+.005,'g-')     %constant wave speed -- +.005 to .02
...
ax.LineWidth = 1.5;
%title("testing various ppw w error as function of \omega")
...
legend([qw{:}], {"ppw = 5","ppw = 10","ppw = 20",'$\frac{1}{\omega}$'}, 'location', 'best','FontSize',20)
