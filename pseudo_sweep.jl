...
function rhs!(du, u, p, t)
    OMEGA, ORDER, invMA = p
    c, d_inv_c2, d2_inv_c2 = p.c_data
    
    k = OMEGA ./ c
    lambda_1_u = apply_lambda_1(ORDER, invMA, k, u)
    lambda_0_u = (Diagonal(k.^2) + invMA) \ (-0.25 * OMEGA^2 * d_inv_c2 .* u)

    tmp_1 = -0.25 * ((Diagonal(k.^2) + invMA) * (OMEGA^2 * d2_inv_c2 .* u) - (OMEGA^2 * d_inv_c2).^2 .* u)
    tmp_1 = (Diagonal(k.^2) + invMA) \ ((Diagonal(k.^2) + invMA) \ tmp_1)
    tmp_2 = (Diagonal(k.^2) + invMA) \ ((Diagonal(k.^2) + invMA) \ ((0.25 * OMEGA^2 * d_inv_c2).^2 .* u)) 
    lambda_n1_u = (Diagonal(k.^2) + invMA) \ apply_lambda_1(ORDER, invMA, k, 0.5 * (tmp_1 + tmp_2))
...
function pseudo_sweep(pseudodiff_params, N, num_elements; 
                      domain_width=1, domain_height=1)
    ...
    c_data = (; c=C[:,1], d_inv_c2=d_inv_C2[:,1], d2_inv_c2=d2_inv_C2[:,1])

    forcing = (x, y) -> 0.0 # !!! fix 
    f = forcing.(x',y)
    p = (; OMEGA, ORDER, invMA, c_data)
    ...
    for i in Nx:-1:2
        ...
        k1 = -rhs(u[:,i], p, x[i]) + f[:,i]
        u1 = u[:,i] + dx_backwards * k1

        @. c_data.c = C[:, i-1]
        @. c_data.d_inv_c2 = d_inv_C2[:, i-1]
        @. c_data.d2_inv_c2 = d2_inv_C2[:, i-1]
        k2 = -rhs(u1, p, x[i-1]) + f[:,i-1]
    ...
    for i in 1:Nx-1
        ...
        @. c_data.d_inv_c2 = d_inv_C2[:, i]
        @. c_data.d2_inv_c2 = d2_inv_C2[:, i]
        k1 = rhs(u[:,i], p, x[i]) + u_backwards[:,i]
        u1 = u[:,i] + dx * k1

        @. c_data.c = C[:, i+1]
        @. c_data.d_inv_c2 = d_inv_C2[:, i+1]
        @. c_data.d2_inv_c2 = d2_inv_C2[:, i+1]
        k2 = rhs(u1, p, x[i+1]) + u_backwards[:,i+1]
        u2 = u1 + dx * k2
