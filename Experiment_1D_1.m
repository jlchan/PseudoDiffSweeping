...
for i = 5:5:40
    ...
    loglog(dt,max(abs(err_RK4)),'rs','MarkerSize',10)
    hold on
...
for i = 5:2.5:40
    ...
    %t2 = linspace(0, FinalTime, Nsteps);
    loglog(dt,max(abs(err_RK2)),'ko','MarkerSize',10)
...
%Sweep using Heun's method
    ...
    for j = 1:n-1
        ...
        v(j+1) = 0.5 * (v(j) + v2);     %IS THIS CORRECT?
...
dt_backwards = -dtime;    %reverse dt to get back sweep
