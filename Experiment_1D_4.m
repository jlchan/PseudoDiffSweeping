...
%varying ppw, testing 4 values of omega (25pi, 50pi, 100pi, 200pi)
...
for i = 5:5:50
    ...
    semilogy(ppw,max(abs(err_RK4)),'ko','MarkerSize',10)
    hold on
...
for i = 5:5:50
    ...
    semilogy(ppw,max(abs(err_RK4)),'co','MarkerSize',10)
...
for i = 5:5:50
    ...
    semilogy(ppw,max(abs(err_RK4)),'bo','MarkerSize',10)
...
for i = 5:5:50
    ...
    semilogy(ppw,max(abs(err_RK4)),'ro','MarkerSize',10)

end

xlabel('points per wavelength','FontSize',20)
ylabel('max(abs(error))','FontSize',201)
ax = gca;
ax.FontSize = 15;
ax.TickLength = [0.02, 0.05];
ax.LineWidth = 1.5;
ax.YLim = [0 .1]
%title("testing various omega w error as function of ppw")
...
legend([qw{:}], {'\omega = 25 \pi',"\omega = 50 \pi","\omega = 100 \pi","\omega = 200 \pi"}, 'location', 'best', 'FontSize',20)
